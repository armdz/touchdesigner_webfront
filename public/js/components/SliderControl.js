...
export class SliderControl extends BaseControl {
  ...
  _renderVertical() {
    const el = document.createElement('div');
    el.className = 'control slider-control slider-control--vertical vslider-control';
    el.innerHTML = `
      <div class="vslider-control__wrap">
        <span class="control__id">${this.id}</span>
        <span class="vslider-control__label">${this.label}</span>
        ...
        <span class="vslider-control__val" data-val>${this._fmt(this.value)}</span>
      </div>
    `;
  ...
  toJSON() {
    return { ...super.toJSON(), min: this.min, max: this.max, step: this.step, orientation: this.orientation };
  }
}
